...
public class MemberDao {
	...
	public MemberDto getUserData(String id) {
		String sql = " SELECT TP_M_SEQ," + " TP_M_ID, " + " TP_M_PWD, " + " TP_M_NAME, " + " TP_M_EMAIL, "
				+ " TP_M_AUTH, " + " TP_M_JOINDATE, " +" TP_M_USERPIC " + " FROM TP_MEMBER " + " WHERE TP_M_ID=? ";
	...
	public MemberDto login(MemberDto dto) {
		...
		String sql = " SELECT TP_M_ID, TP_M_PWD, TP_M_NAME, TP_M_EMAIL, " + " TP_M_DEL, TP_M_AUTH, "
				+ " TP_M_USERPIC " + " FROM TP_MEMBER "
		...
		try {
			...
			if (rs.next()) {
				...
				String userPic = rs.getString(7);
				mem = new MemberDto(id, pwd, name, email, auth, del, userPic);
